...
public class WiimoteAdapter extends WiiRemoteAdapter
{
  //debug = true for testing and outputting data, false otherwise
  ...
  public WiimoteAdapter(WiiRemote remote)
  {
    this.remote = remote;
  }
  ...
  public void IRInputReceived(WRIREvent evt)
  {
    for (IRLight light : evt.getIRLights())
    {
      if (light != null)
      {
        lightX = 1024.00 - light.getX()*1024;
        lightY = light.getY()*768;
        Connect.irX = lightX;
        Connect.irY = lightY;
        if (debug==true)
        {
  ...
  public void buttonInputReceived(WRButtonEvent evt)
  {
    ...
    if (evt.wasPressed(WRButtonEvent.B))
    {
      ...
      Connect.buttonpressed = "B";
      if (Connect.go==false) Connect.go = true;
    } 
    ...
    try
    {
      ...
      {
        if (Connect.prebuf != null)remote.playPrebufferedSound(Connect.prebuf, WiiRemote.SF_PCM8S);
      }
